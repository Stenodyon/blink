...
pub fn RenderCopy(renderer: Renderer, texture: Texture,
        dest: ?vec.Rect, src: ?vec.Rect) c_int
{
    const sdl_dest = if (dest) |r| r.to_sdl() else null;
    const sdl_src  = if (src)  |r| r.to_sdl() else null;
    return c.SDL_RenderCopy(renderer, texture,
...
pub fn RenderCopyEx(
        renderer: Renderer,
        texture: Texture,
        dest: ?vec.Rect, src: ?vec.Rect,
        angle: f64,
        center: ?*vec.Vec2i,
        flip: RendererFlip) c_int
{
    const sdl_dest = if (dest) |r| r.to_sdl() else null;
    const sdl_src  = if (src)  |r| r.to_sdl() else null;
    const sdl_center = if (center) |p| p.to_sdl() else null;
    return c.SDL_RenderCopyEx(renderer, texture,
            @ptrCast(?[*]const Rect, &sdl_dest),
            @ptrCast(?[*]const Rect, &sdl_src),
            angle,
